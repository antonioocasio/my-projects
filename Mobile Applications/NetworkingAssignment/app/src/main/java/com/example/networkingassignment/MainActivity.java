...
public class MainActivity extends AppCompatActivity {
    ...
    private static class DownloadData extends AsyncTask<String, Void, ArrayList<DNA>> {
        ...
        @Override
        protected void onPostExecute(ArrayList<DNA> dna) {
            MainActivity activity = activityReference.get();

            if (activity == null || activity.isFinishing()) return;

            // once activity is finished progress bar goes away
            activity.progressBar.setVisibility(View.GONE);

            if(dna != null){
                activity.dnaAdapter.setDnaData(dna);
            }
        }
    }
}
